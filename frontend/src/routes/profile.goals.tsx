      ...
      <div className="px-4 lg:px-8 py-6 max-w-2xl mx-auto space-y-6 pb-12">
        ...
        <div className="card-frosted p-5">
          <div className="flex justify-between items-center mb-4">
            ...
            >
              {editingGoals ? (
                <Check size={16} style={{ color: c.sunGlare }} />
              ) : (
                <Edit2 size={14} />
              )}
            </button>
                  ...
                  return (
                    <span
                      ...
                      style={{
                        background: c.sunGlareBg,
                        color: c.sunGlare,
                        border: `1px solid ${c.sunGlare}22`,
                      }}
                    >
                      {g.label}
                    </span>
            ...
            <div className="flex gap-4">
              <div className="flex-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setOnboarding({ currentWeight: e.target.value ? Number(e.target.value) : null })
                  }
              ...
              <div className="flex-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setOnboarding({ goalWeight: e.target.value ? Number(e.target.value) : null })
                  }
                  className="w-full rounded-lg px-4 py-3 text-lg font-bold focus:outline-none transition-colors"
                  style={{
                    background: c.inputBg,
        ...
        <div className="card-frosted p-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs mt-1" style={{ color: c.textSecondary }}>
              How many times you want to work out
            </p>
          </div>
          <div
            className="flex items-center gap-4 bg-black/5 dark:bg-white/5 rounded-2xl p-1.5"
            style={{ border: `1px solid ${c.chipBorder}` }}
          >
            <button
              onClick={() =>
                setOnboarding({
                  weeklySessionTarget: Math.max(1, onboarding.weeklySessionTarget - 1),
                })
              }
            ...
            <button
              onClick={() =>
                setOnboarding({
                  weeklySessionTarget: Math.min(7, onboarding.weeklySessionTarget + 1),
                })
              }
              className="w-10 h-10 rounded-xl grid place-items-center transition-colors active:scale-95 bg-white dark:bg-[#2A2A26] shadow-sm"
              style={{ color: c.textPrimary }}
        ...
        {/* Fitness Level */}
        <div className="card-frosted p-5">
          <h2 className="font-bold text-[15px] mb-4" style={{ color: c.textPrimary }}>
            Fitness level
          </h2>
          <div className="flex flex-wrap gap-2">
